...
```bash
python3 ~/.kiro/skills/logd-reference/scripts/extensions/obs_scan.py load-previous --service {service}
```
...
**Compare with previous scan + known-error-patterns skill:**
...
Check the known-error-patterns skill for baseline severity and context. Override severity if the pattern is known and documented.
...
```bash
python3 ~/.kiro/skills/logd-reference/scripts/extensions/obs_scan.py save '{JSON}'
```

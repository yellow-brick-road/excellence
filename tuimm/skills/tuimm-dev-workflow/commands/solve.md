...
- If ticket ID provided: gather full ticket context via tuimm_subagent_jira following the tuimm-jira-context-gathering skill — parent, links, comments, attachments. If the ticket has sub-tasks or linked issues, fetch their full details too (second subagent call). Present a summary of the parent AND all sub-tasks/links before proceeding
...
Clone or reuse workspace at `~/.kiro/temp/dev/{TICKET-ID}-{repo_name}/` (see TUIMM_GIT.md § Workspace Conventions):
...
- Via tuimm_subagent_gitlab following tuimm-commit-conventions skill

...
- Get ticket context via tuimm_subagent_jira if linked (follow tuimm-jira-context-gathering skill)
- Clone or reuse workspace at `~/.kiro/temp/mr/mr-{iid}-{repo_name}/` (see TUIMM_GIT.md § Workspace Conventions). If dir exists: fetch + checkout. If not: clone + checkout MR source branch
...
4. Commit via tuimm_subagent_gitlab (follow tuimm-commit-conventions skill)

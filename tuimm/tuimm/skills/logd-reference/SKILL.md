...
name: logd-reference
description: |
  logd — local log daemon and client library at ~/.kiro/tuimm/tools/logd/.
...
```bash
python3 ~/.kiro/tuimm/tools/logd/logd.py start       # start daemon (background)
python3 ~/.kiro/tuimm/tools/logd/logd.py stop        # stop daemon
python3 ~/.kiro/tuimm/tools/logd/logd.py status      # running? entry count? DB size?
python3 ~/.kiro/tuimm/tools/logd/logd.py run         # foreground (debug)
```
...
```bash
# Tail recent logs
python3 ~/.kiro/tuimm/tools/logd/logd.py tail [N]                    # last N entries (default 30)
python3 ~/.kiro/tuimm/tools/logd/logd.py tail --uid XXX              # filter by run uid
python3 ~/.kiro/tuimm/tools/logd/logd.py tail --service autobuild    # filter by service
python3 ~/.kiro/tuimm/tools/logd/logd.py tail --level ERROR          # filter by level
python3 ~/.kiro/tuimm/tools/logd/logd.py tail --last 2h              # last 2 hours
python3 ~/.kiro/tuimm/tools/logd/logd.py tail --json                 # JSON output

# Search
python3 ~/.kiro/tuimm/tools/logd/logd.py search "pattern"            # text search
python3 ~/.kiro/tuimm/tools/logd/logd.py search "failed" --service bg --last 1h

# List runs
python3 ~/.kiro/tuimm/tools/logd/logd.py runs                        # recent execution runs
python3 ~/.kiro/tuimm/tools/logd/logd.py runs --last 7d              # last 7 days
```
...
- Database: `~/.kiro/tuimm/tools/logd/logs.db` (SQLite WAL mode)

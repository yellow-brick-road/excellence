...
| **Command** | Single-agent targeted action. Lives in `excellence/commands/`, loaded as skill via `skill://` in agent JSON. Triggered with `$name` prefix |
| **Excellence** | Cross-cutting function within DP&T. Accelerator, not a delivery team |
| **Excellence Default** | Concierge agent — guides users to the correct specialist agent, executes shared commands (jira, weblate) directly |
...
| **Prompt** | Multi-domain orchestration owned by Excellence Default. Coordinates 2+ specialist agents (none created yet) |
...
| **Template** | Output format definition. Lives in `excellence/templates/`. Consumed by agents/commands for structured output |
